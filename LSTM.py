...
def mard(y_ture,y_predict):
    a=np.average(abs(y_ture-y_predict)/y_ture)
    b=np.average(abs(y_ture-y_predict))
...
# 加载数据
series = read_csv('CGM_1day_value_new_test.csv', header=0, parse_dates=[0], index_col=0, squeeze=True, date_parser=parser)
# 最后N条数据作为测试数据
testNum = 96
...
mad = mard(raw_values[-testNum:], predictions)
print('Test RMSE: %.3f' % RMSE)
print('Test MARD: %.3f' % mad)
# 作图展示
pyplot.plot(raw_values[-testNum:],label='real_value')
pyplot.plot(predictions,label='predict_value')
pyplot.show()
